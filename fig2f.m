...
average_population_transfers = zeros(length(w), length(d), length(p));
...
for i = 1:length(w)
    ...
    T = (0.4 - (w_val-2)*0.05);
    r = 1.15*w_val*w_val/4;
    r1 = w_val*w_val/4*0.6;
    exp_term = exp(-t1/T);
    exp_t1_over_T = exp(t1/T);
    y_val1_arr(i) = r*2*(exp_term - exp_t1_over_T)/(exp_term + exp_t1_over_T);
    y_val2_arr(i) = -r*2*(exp_term - exp_t1_over_T)/(exp_term + exp_t1_over_T);
    ...
    for j = 1:length(d)
        ...
        for k = 1:length(p)
            p_val = p(k);
            average_population_transfers(i, j, k) = eve2(N, T, t1, t2, r, r1, w_val, d_val, d0, p_val);
        end
...
plot(w, y_val1_arr, 'k--', 'LineWidth', 1.5, 'DisplayName', 'y_{val1}');
plot(w, y_val2_arr, 'k--', 'LineWidth', 1.5, 'DisplayName', 'y_{val2}');
