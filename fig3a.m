...
d = linspace(0, B/2, 10);
T = linspace(0.05, 0.4, 5);

% SAP1's F
ave1 = zeros(1, length(w));
final_transfer1 = zeros(length(w), length(d));
...
for i = 1:length(w)
    ...
    for j = 1:length(d)
        ...
        final_transfer1(i, j) = eve1(N, (0.2-(w_val-3)*0.02), t1, t2, B * (0.2-(w_val-3)*0.02) / t2, w_val, d_val);
    end
    ave1(i) = mean(final_transfer1(i, :));
end

% SAP2's F
ave2 = zeros(1, length(w));
av2 = zeros(length(w), length(T));
avd2 = zeros(length(w), length(T), 8);
current_avg2 = zeros(length(w), length(T), 8, length(p));
final_transfer2 = zeros(length(w), length(T), 8, length(p), length(d));

for i = 1:length(w)
    w_val = w(i);
    r = linspace(w_val-1.5, w_val+0.5, 8);
    for n = 1:length(T)
        T_val = T(n);
        for m = 1:length(r)
            r_val = r(m);
            for j = 1:length(p)
                ...
                for k = 1:length(d)
                    ...
                    final_transfer2(i, n, m, j, k) = eve2(N, T_val, t1, t2,  r_val , B/2 * T_val / t2, w_val, d_val, d0_val, p_val);
                end
...
% SAP3's F
ave3 = zeros(1, length(w));
av3 = zeros(length(w), length(T));
avd3 = zeros(length(w), length(T), 10);
current_avg3 = zeros(length(w), length(T), 10, length(p));
final_transfer3 = zeros(length(w), length(T), 10, length(p), length(d));

for i = 1:length(w)
    w_val = w(i);
    r = linspace(w_val-2, w_val+0.5, 10);
    for n = 1:length(T)
        T_val = T(n);
        for m = 1:length(r)
            r_val = r(m);
            for j = 1:length(p)
                ...
                for k = 1:length(d)
                    ...
                    final_transfer3(i, n, m, j, k) = eve3(N, T_val, t1, t2,  r_val , B/3 * T_val / t2, w_val, d_val, d0_val, p_val);
                end
...
% Fit curve
coefficients_ave1 = polyfit(w, ave1, 3);
y_fit_ave1 = polyval(coefficients_ave1, w);

coefficients_ave2 = polyfit(w, ave2, 3);
y_fit_ave2 = polyval(coefficients_ave2, w);

coefficients_ave3 = polyfit(w, ave3, 3);
y_fit_ave3 = polyval(coefficients_ave3, w);

figure;
hold on;

h_ave1_scatter = scatter(w, ave1, 75, 'x',...  
    'MarkerEdgeColor', [0.1 0.4 0.7],...  
    'LineWidth', 2.0);  

h_ave1_fit = plot(w, y_fit_ave1, ':',...  
    'Color', [0.2 0.8 0.2],... 
    'LineWidth', 2.5);

h_ave2_scatter = scatter(w, ave2, 75, '+',...
    'MarkerEdgeColor', [0.1 0.5 0.9],...
    'LineWidth', 2.0);

h_ave2_fit = plot(w, y_fit_ave2, '--',...
    'Color', [0.1 0.6 1.0],...  
    'LineWidth', 2.5);

h_ave3_scatter = scatter(w, ave3, 75, 'o',...
    'MarkerFaceColor', [0.9 0.2 0.1],... 
    'MarkerEdgeColor', 'k',...
    'LineWidth', 1.5);

h_ave3_fit = plot(w, y_fit_ave3, '-.',...
    'Color', [1.0 0.3 0.2],...  
    'LineWidth', 2.5);
