...
r = linspace(2, 4.5, 10);
T = linspace(0.05, 0.25, 3);

% SAP1's F
ave1 = zeros(1, length(B));
final_transfer1 = zeros(length(B), 10);

for i = 1:length(B)
    B_val = B(i);
    d = linspace(0, B_val/2, 10);
    for j = 1:length(d)
        d_val = d(j); 
        N = round(1000 * (t2 + 2 * t1));
        final_transfer1(i, j) = eve1(N, (0.2-(w-3)*0.02), t1, t2, B_val * (0.2-(w-3)*0.02) / t2, w, d_val);
    end
    ave1(i) = mean(final_transfer1(i, :));
end

% SAP2's F
ave2 = zeros(1, length(B));
av2 = zeros(length(B), length(T));
avd2 = zeros(length(B), length(T), length(r));
current_avg2 = zeros(length(B), length(T), length(r), length(p));
final_transfer2 = zeros(length(B), length(T), length(r), length(p), 10);
        ...
        for m = 1:length(r)
            ...
            for j = 1:length(p)
                ...
                for k = 1:length(d)
                    d_val = d(k); 
                    N = round(1000 * (t2 + 2 * t1));
                    d0_val = B_val/2 - r_val * 2 * (exp(-t1/T_val) - exp(-(-t1)/T_val)) / (exp(-t1/T_val) + exp(-(-t1)/T_val));
                    final_transfer2(i, n, m, j, k) = eve2(N, T_val, t1, t2,  r_val , B_val/2 * T_val / t2, w, d_val, d0_val, p_val);
                end
...
% SAP3's F
ave3 = zeros(1, length(B));
av3 = zeros(length(B), length(T));
avd3 = zeros(length(B), length(T), length(r));
current_avg3 = zeros(length(B), length(T), length(r), length(p));
final_transfer3 = zeros(length(B), length(T), length(r), length(p), 10);
        ...
        for m = 1:length(r)
            ...
            for j = 1:length(p)
                ...
                for k = 1:length(d)
                    d_val = d(k); 
                    N = round(1000 * (t2 + 2 * t1));
                    d0_val = B_val/3 - r_val * 2 * (exp(-t1/T_val) - exp(-(-t1)/T_val)) / (exp(-t1/T_val) + exp(-(-t1)/T_val));
                    final_transfer3(i, n, m, j, k) = eve3(N, T_val, t1, t2,  r_val , B_val/3 * T_val / t2, w, d_val, d0_val, p_val);
                end
...
% Fit curve
coefficients_ave1 = polyfit(B, ave1, 3);
y_fit_ave1 = polyval(coefficients_ave1, B);  

coefficients_ave2 = polyfit(B, ave2, 3);
y_fit_ave2 = polyval(coefficients_ave2, B);  

coefficients_ave3 = polyfit(B, ave3, 3);
y_fit_ave3 = polyval(coefficients_ave3, B); 

figure;
hold on;

h_ave1_scatter = scatter(B, ave1, 75, 'x',...  
    'MarkerEdgeColor', [0.1 0.4 0.7],...  
    'LineWidth', 2.0);  

h_ave1_fit = plot(B, y_fit_ave1, ':',...  
    'Color', [0.2 0.8 0.2],... 
    'LineWidth', 2.5);

h_ave2_scatter = scatter(B, ave2, 75, '+',...
    'MarkerEdgeColor', [0.1 0.5 0.9],...
    'LineWidth', 2.0);

h_ave2_fit = plot(B, y_fit_ave2, '--',...
    'Color', [0.1 0.6 1.0],...  
    'LineWidth', 2.5);

h_ave3_scatter = scatter(B, ave3, 75, 'o',...
    'MarkerFaceColor', [0.9 0.2 0.1],... 
    'MarkerEdgeColor', 'k',...
    'LineWidth', 1.5);

h_ave3_fit = plot(B, y_fit_ave3, '-.',...
    'Color', [1.0 0.3 0.2],...  
    'LineWidth', 2.5);
